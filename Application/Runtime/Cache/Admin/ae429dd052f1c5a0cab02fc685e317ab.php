                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="table-responsive">
                            <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                ...
                                <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr class="new_<?php echo ($v["admin_id"]); ?>">
                                        <td class="center">
                                            <label>
                                                <input type="checkbox" class="ace" />
                                                <span class="lbl"></span>
                                        ...
                                        <td class="time_<?php echo ($v["news_id"]); ?>">
                                           <?php echo ($v["issue_time"]); ?>


                                        </td>
                                        <td class="hidden-480"><?php echo ($v["seller_lev"]); ?></td>
                                        <td><?php echo ($v["admin_name"]); ?></td>

                                        <td class="hidden-480 icon_<?php echo ($v["news_id"]); ?>">
                                            <?php if($v["check_status"] == 1): ?><span class="label label-sm label-info arrowed arrowed-righ">发布</span>
                                                <?php elseif($v["check_status"] == 2): ?>
                                                <span class="label label-danger arrowed">拒绝</span>
                                                <?php elseif($v["check_status"] == 0): ?>
                                                <span class="label label-warning arrowed arrowed-right">待审核</span><?php endif; ?>
                                        </td>

                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                <?php if($admin_id == 1): if($v["check_status"] == 0): ?><a onclick="pass(<?php echo ($v["news_id"]); ?>)" class="blue pass_<?php echo ($v["news_id"]); ?>" >
                                                            <i class="icon-ok bigger-130"></i>
                                                        </a>
                                                        <a class="green edit_<?php echo ($v["news_id"]); ?>" href="<?php echo U('News/editNews' , array('id' => $v['news_id']));?>">
                                                            <i class="icon-pencil bigger-130"></i>
                                                        </a><?php endif; ?>
                                                    <a onclick="url(<?php echo ($v["news_id"]); ?>)" class="yellow url_<?php echo ($v["news_id"]); ?>" >
                                                        <i class="normal-icon icon-eye-open orange2 bigger-130"></i>
                                                    </a>
                                                    <?php if($v["check_status"] == 0): ?><a onclick="del(<?php echo ($v["news_id"]); ?>)" class="red del_<?php echo ($v["news_id"]); ?>" >
                                                            <i class="icon-trash bigger-130"></i>
                                                        </a><?php endif; ?>

                                                    <?php else: ?>
                                                    <?php if($type == 'needCheck'): if($v["check_status"] == 0): ?><a onclick="pass(<?php echo ($v["news_id"]); ?>)" class="blue pass_<?php echo ($v["news_id"]); ?>" >
                                                                <i class="icon-ok bigger-130"></i>
                                                            </a>
                                                            <a class="green edit_<?php echo ($v["news_id"]); ?>" href="<?php echo U('News/editNews' , array('id' => $v['news_id']));?>">
                                                                <i class="icon-pencil bigger-130"></i>
                                                            </a><?php endif; endif; ?>
                                                    <?php if($type == 'noCheck'): ?><a class="green edit_<?php echo ($v["news_id"]); ?>" href="<?php echo U('News/editNews' , array('id' => $v['news_id']));?>">
                                                            <i class="icon-pencil bigger-130"></i>
                                                        </a><?php endif; ?>
                                                    ...
                                                    <?php if($type == 'needCheck'): if($v["check_status"] == 0): ?><a onclick="del(<?php echo ($v["news_id"]); ?>)" class="red del_<?php echo ($v["news_id"]); ?>" >
                                                                <i class="icon-trash bigger-130"></i>
            ...
            $.ajax({
                ...
                success : function (e) {
                    if (e.res == 1){
                        ...
                        <?php if($is_top == 1): ?>$('.time_' + id).html(time1);
                            $('.icon_' + id).html('<span class="label label-sm label-info arrowed arrowed-righ">发布</span>');
                            <?php else: ?>
                            $('.icon_' + id).html('<span class="label label-warning arrowed arrowed-right">待上级审核</span>');<?php endif; ?>
