                            ...
                            <div class="">
                                <div class="widget-main">
                                        ...
                                        <div class="step-pane active" id="step1">
                                            ...
                                            <form class="form-horizontal" id="validation-form" method="">
                                                <div class="form-group">
                                                    <label class="control-label col-xs-12 col-sm-3 no-padding-right" for="sr_realname">真实姓名:</label>

                                                    <div class="col-xs-12 col-sm-9">
                                                        <div class="clearfix">
                                                            <input type="text" value="<?php echo ($info["sr_realname"]); ?>" name="sr_realname" id="sr_realname" class="col-xs-12 col-sm-6" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="space-2"></div>
                                                <div class="form-group">
                                                    <label class="control-label col-xs-12 col-sm-3 no-padding-right" for="sr_mobile">电话:</label>

                                                    <div class="col-xs-12 col-sm-9">
                                                        <div class="clearfix">
                                                            <input type="text" value="<?php echo ($info["sr_mobile"]); ?>" name="sr_mobile" id="sr_mobile" class="col-xs-12 col-sm-6" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="space-2"></div>
                                                <div class="form-group">
                                                    <label class="control-label col-xs-12 col-sm-3 no-padding-right" for="sr_idcard">身份证:</label>

                                                    <div class="col-xs-12 col-sm-9">
                                                        <div class="clearfix">
                                                            <input type="text" value="<?php echo ($info["sr_idcard"]); ?>" name="sr_idcard" id="sr_idcard" class="col-xs-12 col-sm-6" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="space-2"></div>
                                                ...
                                                <div class="form-group">
                                                    <label class="control-label col-xs-12 col-sm-3 no-padding-right" for="state">经销商等级：</label>

                                                    <div class="col-xs-12 col-sm-9">
                                                        <select id="state" name="lev" class="select2 col-sm-3" data-placeholder="Click to Choose...">

                                                            <?php if(is_array($lev)): foreach($lev as $k=>$v): ?><option value="<?php echo ($k); ?>" <?php if(($info["sr_lev"]) == $k): ?>selected<?php endif; ?>><?php echo ($v); ?></option><?php endforeach; endif; ?>
                                    ...
                                    <div class="clearfix col-md-offset-3 col-md-9">
                                        <button class="btn btn-info" onclick="edit(<?php echo ($info["id"]); ?>)">
                                            <i class="icon-ok"></i>
                                            修改
                                        </button>
...
<script>
    function edit(id){
        var name = $('#sr_realname').val();
        var phone = $('#sr_mobile').val();
        var idcard = $('#sr_idcard').val();
        var lev = $('select[name=lev] option:selected').val();

        if (name == ""){
            swal({title:"提示",text:"姓名不能为空",type:"error"})
        }
        if (phone == ""){
            swal({title:"提示",text:"手机号不能为空",type:"error"})
        }
        if (idcard == ""){
            swal({title:"提示",text:"身份证信息不能为空",type:"error"})
        }
        if (lev == ""){
            swal({title:"提示",text:"请选择经销商等级",type:"error"})
        }

        $.ajax({
            url : "<?php echo U('Seller/updateUser');?>",
            type : "POST",
            dataType : "JSON",
            data : {name : name , phone : phone , idcard : idcard , lev : lev , id : id},
            success : function (e) {
                if (e.res == 1){
